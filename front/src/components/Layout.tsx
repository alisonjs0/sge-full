...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen  flex">
      ...
      {/* Sidebar */}
      <div className={`
        fixed inset-y-0 left-0 z-50 w-64 h-screen bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 overflow-y-auto
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200">
          <Link className="flex items-center gap-3" href={"/"}>
            <Shield className="h-8 w-8 text-red-600"/>
            <span className="text-xl font-bold text-gray-900">SGE</span>
          </Link>
          <button
            onClick={() => setSidebarOpen(false)}
          ...
          <div className="relative group">
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <div className="text-xs text-gray-500">{user?.cpf || 'Carregando...'}</div>
              </div>
            </button>
            {userMenuOpen && (
              <div 
